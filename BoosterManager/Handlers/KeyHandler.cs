...
namespace BoosterManager {
	internal static class KeyHandler {
		...
		internal const ulong KeyClassID = 101785959;
		...
		internal static async Task<string> GetKeyCount(Bot bot) {
			...
			try {
				inventory = await bot.ArchiWebHandler.GetInventoryAsync(appID: KeyAppID, contextID: KeyContextID).Where(item => item.ClassID == KeyClassID).ToHashSetAsync().ConfigureAwait(false);
			} catch (Exception e) {
